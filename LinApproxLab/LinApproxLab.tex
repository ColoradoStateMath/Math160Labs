...
As a warning: not all approximations are over estimates! This just came about from this example.
\end{question}
...
\begin{example}%\label{exam:Periodicity of a pendulum}
...
\begin{tikzpicture}
    ...
    \fill (bob) circle (0.2);
    \node[below left=5pt of {(1.5,-2.5)}] {$M$};
...
by angle of displacement $\theta$. If we put the pendulum
at a certain $\theta$ and let go, the pendulum will swing back
...
Our goal is to solve this differential equation to find what function
$\theta$ is. Once we find $\theta$, we can find the period of the pendulum's swing.

Manipulate the above equation so that '$0$' is on one side and
the $\dfrac{d^2 \theta}{d t^2}$ term is positive.

\[
0 = \answer[given]{m L^2} \dfrac{d^2 \theta}{d t^2}  + \answer[given]{m g L \sin(\theta)}
\]

Note that $L,m,g>0$, so we can divide the equality by of these
to get an equivalent statement. Manipulate the above equality 
so that the coefficient of $\dfrac{d^2 \theta}{d t^2}$ is $1$.
...
to the differential equation with depend on the length of the string $L$.
\end{feedback}
\end{question}

\begin{question}
Does the above relationship depend on the gravitional constant?
\begin{multipleChoice}
\choice[correct]{Yes}
\choice{No}
\end{multipleChoice}
...
to the differential equation with depends on earth's gravitational
constant $g$.
\end{feedback}
\end{question}
...
A.K.A. the position and velocity of ball's path doesn't depend
...
Now we will utilize the fact that $\theta$, the position of the ball,
...
\begin{prompt}
\[
\dfrac{d^2 \theta}{d t^2}  = \answer[given]{\dfrac{-g}{L} \sin(\theta)}
\]
\end{prompt}
%%%% Make this into a better format (above)
...
\begin{question}
What is a linear approximation of $\sin(\theta)$ at the above theta value?
...
$|\theta|\leq \frac{\pi}{12} = 15^\circ$! So "small angles in
this context is for angles that deviate no more that $15^\circ$ from equilibrium.
...
\begin{question}
Which polynomial $\theta = f(t)$ has the property that it's second
derivative is $-1$ times itself ($\dfrac{d^2 \theta}{d t^2} = f''(t) = -f(t)$?
...
This is our equilibrium solution! This function is $0$ at $t=0$.
...
\begin{question}
Since we are simulating motion when letting go of the pendulum
from a stand still, which of functions you picked above would
accurately simulate this situation?
\begin{selectAll}
\choice{$\sin(t)$}
\choice[correct]{$\cos(t)$}
\choice{$\tan(t)$}
...
\end{hint}
\begin{hint}
We want a function $f(t)$ that $f'(t) = 0$ at $t=0$.
\end{hint}
\end{question}

We're almost there! We know what the function generally looks like!
...
\begin{align*}
    \dfrac{d^2}{d t^2}(\theta) &= \dfrac{d^2}{d t^2}(\cos(a x + b))\\

...
\begin{example}
Now we will determine what range of $a$ values we can use to ensure
that the linear approximation $\l(x)$ of $f(x)=\sqrt[3]{x}$ 
at $x=a$ will be within a tolerance of $0.01$ of $\sqrt[3]{50}$
when approximating $\sqrt[3]{50}$.

Recall that $\l(x) = f'(a)(x-a) + f(a)$.\\

We want to konw what $a$ values for $\l(x)$ have the property that $|\l(50) - \sqrt[3]{50}|<0.01$.

Since $\sqrt[3]{50} \approx 3.684031$, we need to find for what $a$ values $\l(50) = 3.694031$, and $\l(50) = 3.674031$

\[]
\graph{l\left(a\right)=\ \frac{1}{3a^{\frac{2}{3}}}\left(50-a\right)\ +\ a^{\frac{1}{3}}, y=50^{1/3} + 0.01, y=50^{1/3} - 0.01}
\]

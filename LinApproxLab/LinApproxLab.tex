...
to change $m$ and $b$ so that $t(x)$ is thelinear approximation you
found above. Remember to round to 6 decimal places.
...
at $x=12$ (for example) by typing $t(12)$ into one of the blank boxes below $t(x)$.
You must have $t(x)$ defined on a different line. The graph of you line
should be visible on the graph in order to do this.
\end{expandable}
\begin{expandable}
If $t(x)$ is a linear approximation of a function and you want to approximate
the function at $14$, type $t(14)$ into one of the boxes that doesn't define what $t(x)$ is.
\end{expandable}

\[
\graph[panel]{t(x)=mx+b}
\]
\hrule

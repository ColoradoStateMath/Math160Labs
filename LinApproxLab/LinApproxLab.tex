...
\begin{example}
Below is the graph of $f(x) = 1/8x^3 + (3/8)x^2 -3/4 - 2$ and the line tangent to $f(x)$ at $x=a$. Feel free to explore how the tangent line (in purple) changes as $a$ varies by using the slider.
\[
...
\begin{example}
\textbf{Example.}   Consider a pendulum with ball $B$ with mass $m$ hanging from a string
...
\textbf{Warning: Leave spaces between any variables being multiplied and type `theta' when you want $\theta$.}
...
is $1$. Be sure to simplify and fractions.
